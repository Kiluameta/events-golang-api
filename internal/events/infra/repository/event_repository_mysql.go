...
import (
	...
	"github.com/Kiluameta/events-golang-api/internal/events/domain"
...
// LIST EVENTS
func (m *mysqlEventRepository) ListEvents() ([]domain.Event, error) {
	query := `
		SELECT 
...
// FIND EVENT
func (m *mysqlEventRepository) FindEventByID(eventID string) (*domain.Event, error) {
	query := `
		SELECT 
			e.id, e.name, e.location, e.organization, e.rating, e.date, e.image_url, e.capacity, e.price, e.partner_id,
			s.id, s.event_id, s.name, s.status, s.ticket_id,
...
// CREATE EVENT
func (m *mysqlEventRepository) CreateEvent(event *domain.Event) error {
	query := `
		INSERT INTO events (id, name, location, organization, rating, date, image_url, capacity, price, partner_id)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
	_, err := m.db.Exec(query, event.ID, event.Name, event.Location, event.Organization, event.Rating, event.Date.Format("2006-01-02 15:04:05"), event.ImageURL, event.Capacity, event.Price, event.PartnerID)
	return err
}

// FIND SPOT
func (m *mysqlEventRepository) FindSpotByID(spotID string) (*domain.Spot, error) {
	query := `
		SELECT
			s.id, s.event_id, s.name, s.status, s.ticket_id,
			t.id, t.event_id, t.spot_id, t.ticket_kind, t.price
		...
		WHERE s.id = ?
	`
	row := m.db.QueryRow(query, spotID)
...
func (m *mysqlEventRepository) CreateTicket(ticket *domain.Ticket) error {
	query := `
		...
		VALUES (?, ?, ?, ?, ?)
	`
	_, err := m.db.Exec(query, ticket.ID, ticket.EventID, ticket.Spot.ID, ticket.TicketType, ticket.Price)
	return err
}

// RESERVE SPOT
func (m *mysqlEventRepository) ReserveSpot(spotID, ticketID string) error {
	query := `
		UPDATE spots
		SET status = ?, ticket_id = ?
		WHERE id = ?
	`
	_, err := m.db.Exec(query, domain.SpotStatusReserved, ticketID, spotID)
	return err
}

// FIND SPOT BY EVENTID
...
// FIND SPOT BY NAME
func (m *mysqlEventRepository) FindSpotByName(eventID, name string) (*domain.Spot, error) {
	query := `
		SELECT 
			s.id, s.event_id, s.name, s.status, s.ticket_id,
			t.id, t.event_id, t.spot_id, t.ticket_kind, t.price
	...
	`
	row := m.db.QueryRow(query, eventID, name)
